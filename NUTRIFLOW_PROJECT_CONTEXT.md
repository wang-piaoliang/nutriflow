...
- 添加照片的 📷 按钮：**36×36 的实线小方块**，放在 `.photo-grid` 的最后一格，靠左、垂直居中。这个尺寸经过两轮修正才定下来——先是独占一整行右对齐（左边一大片空白，用户嫌照片上方空档大），改成撑满整格的虚线大方框后用户又嫌太大。所以两条都要守住：**位置在网格里**（不要拆回单独一行），**尺寸是 36×36**（不要给它 `aspect-ratio:1` 去撑满格子）。没有照片时整块高度约 49px，不会出现一个大空框。
