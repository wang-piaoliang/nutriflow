...
- 现有记录：2026-07-23（午餐、晚餐）、2026-07-22（午餐、晚餐）、2026-07-21（午餐、晚餐）、2026-07-20（午餐、晚餐），均来自同步包，只有食物名称、没有估算量。源数组按日期倒序书写，页面也按日期倒序渲染。
...
- **用户可以在页面上手动补记餐食**：饮食页顶部的「＋ 补记一餐」，选日期和餐次、填食物（空格/逗号/顿号/加号都能当分隔符）。存在 `localStorage` 的 `nutriflow_diet_entries_v1`，和"吃完"状态一样是**设备本地**的，不进 Git、不跨设备。
