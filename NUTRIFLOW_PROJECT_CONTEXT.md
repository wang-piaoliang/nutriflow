...
- 鸡蛋不再是独立一行，而是写在「鱼禽肉蛋」`note` 的**末尾**（先写“可食生重；…不靠单一肉类堆量。”，再写“🥚 其中鸡蛋 1 个，…”）。用户明确要求它不换行、不加粗，且要排在通用说明之后，所以不要再给它单独的元素或字重，也不要挪到句首。
...
- 「购买的食品」已改名为「现有食材」。这份清单会越来越长，所以条目标题压到 14px/500 字重、说明文字 12px、行内边距 10px，目的是一屏多放几件。`.buy-row strong` 和 `.buy-row small` 的覆盖规则必须写在 `.item strong,.buy-row strong,.receipt-line strong` 那两条共享规则**之后**，否则同优先级下 font-size 会被共享规则盖掉。
