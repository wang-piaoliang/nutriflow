...
- 每个展开的小票可添加多张私密照片。照片 Blob 仅保存在添加时所在设备、所在浏览器和同一网址的 IndexedDB（数据库 `nutriflow_private_photos_v1`）中；不写入采购数组、不提交 GitHub、不发布到 GitHub Pages，其他人和其他设备看不到。
