...
- 鸡蛋不再是独立一行，而是写进「鱼禽肉蛋」`note` 的开头（🥚 其中鸡蛋 1 个，…），因为它本来就计入该项总量。用户明确要求它**不换行、不加粗**，所以不要再给它单独的元素或字重。
...
- 添加照片的 📷 按钮是 `.photo-grid` 里的最后一格（`.photo-add`，虚线方框），和缩略图并排。它以前独占一整行、右对齐，左边一大片空白，用户反馈照片上方空档太大，所以并进了网格。不要再把它拆回单独一行。
