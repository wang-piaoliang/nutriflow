...
- 2026-07-25 由用户三张照片录入（约定：多张=多顿，第一张更早）：午餐为番茄虾仁面（`{name:"虾仁", as:"虾"}`、番茄、生菜、金针菇、面，配一杯白饮按 `豆浆` 记）；晚餐为虾仁猪肉煎饺（图 2/3 是同一顿，煎制过程 + 成品），写作 `虾仁猪肉饺子`——名字含「虾/猪肉」会自动归鱼禽瘦肉（正合用户强调的馅料），无需加 `饺` 关键词或 `as`。白饮到底是豆浆/米浆/奶昔未确认；面那顿是否在外吃（照片像现磨豆浆的面馆）也未确认，暂按在家、未加 `place`。
