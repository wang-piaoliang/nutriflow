...
- 2026-07-22 盒马鲜生（大钟寺店）新增一单（`receiptId` `2026-07-22-hema`）：长白萝卜（foodId `radish`，新增图标 🥬 并加入 `purchaseGroupRules` 蔬菜组）、盒马大颗粒虾滑（foodId `shellfish`，2 袋×150g 计 300g，24.28 元）、盒马日日鲜生菜（foodId `leafy`）、鲜香菇（foodId `mushroom`）。乌冬面属主食，按采购过滤规则跳过（但已进饮食记录）。该单整单总价同步包写 42.42 元，而全部 5 件明细相加只有 40.56 元（差 1.86，来源不明），且我们又主动跳过乌冬面，故 `receiptTotalKnown:false`，页面小票总额显示已记 4 件的 31.91 元。虾滑按 300g 计约 80.9 元/kg，成为「食材单价对比」里最贵的一条。
