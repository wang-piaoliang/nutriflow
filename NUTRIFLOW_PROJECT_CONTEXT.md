...
- 鸡蛋不再是独立一行，而是作为 `dailyTargets[].inside` 加粗显示在「鱼禽肉蛋」行内（🥚 其中鸡蛋 1 个），因为它本来就计入该项总量。
...
- 饮食页顶部有一张周总结卡片，按分类统计本周吃到的**不同食物种数**，例如“🥩 鱼禽瘦肉 5 种 / 虾 · 肉丸 · 鸡肉 · 猪肉 · 牛肉”。同一样吃了多次只算一种。
- 分类复用 `dietItemCategoryRules`（含 `icon`），所以和餐次内排序用的是同一套规则，改关键词会同时影响两处。
- 一种都没吃到的分类**照样列出来**并淡显（`.target-row-empty`），显示“本周还没吃到”。空缺本身就是有用的信息，不要为了好看把空分类隐藏掉。
