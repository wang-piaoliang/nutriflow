...
- 2026-09-10：**同一样东西被识别成两条**（用户："甜玉米和土豆都被识别了两遍"）。去重原来两道，都栽在"重叠截图切掉了某一列"上：
  ...
  - 第二道键 = 店 + **foodId** + 规格 + **金额**，而且 `filter(totalPrice > 0)`——**没读到价的那条根本不参与**。重叠区域最常被切掉的恰恰是价格那一列。
  - 于是"名字不一样 + 一条没价"这种最常见的组合，两道全漏。
  - 改法：**金额从"键"降级成"否决票"**。店 + foodId + 规格 + 一天之内先认定是同一笔，只有**两条都读到价、而且对不上**才判成真买了两回；一边没价就合并，并把有价那条的金额捡到留下来的行上（合并不能把钱合没）。
  - 再补**第三道**：规格那一列同样会被切掉（玉米一会儿「2根」一会儿「500g」）。这一道把规格拿掉，改成要求**同一天 + 金额分毫不差**——散装称重的价钱是按重量算的，重量不同价钱不可能一样。
  - **反向的坑**：foodId 太粗，牛嫩肉和牛腱肉都是 `beef`，真是两块不同的肉。所以放宽出来的这两种情况额外要求名字对得上（`looksLikeSameItem`：剥掉规格括号后一个包含另一个）。重叠截图读出来的两个名字总是互相包含，牛嫩肉和牛腱肉互不包含。已加两条反向断言守着。
  - **然后发现了更根上的一条**（用户："没有去重？"）：去重从来只在 `manualPurchases` 内部互相比，可代码里还写死了一批早期采购（`purchases`，7/30 那单盒马就在里面，**土豆（黄心）和水果脆甜玉米正好都在**）。把同一张小票重新拍一遍，识别出来的行和写死的那条永远碰不上面——怎么刷新都是两条。三道去重现在都在 `purchases + manualPurchases` 全集上比，写死的那批删不掉，就让它们永远排在最前面当"留下来的那条"，被判重复的只可能是手工行。
  - 顺带修了个隐蔽的：桶里原来只留第一条候选。写死的那批排最前，只留它的话，后面两条真正互为重复的手工行就永远撞不上了（它们只跟它比日期，隔着一个多月，各自判成"不是同一笔"）。改成一个键存**一组**候选，逐个比。
  - 去重在启动时和每次识别完都会跑，所以刷新一下页面，存量的重复行会自己收掉。离线缓存与版本号升至 v159。
...
- 2026-09-05：**新增「吃掉了多少」：按周/月统计各大类和具体食材的实际消耗量**（用户："统一下我每月、每周吃的每种食材的量…先算大类，再看是否可以点开每大类下面具体的食材量"）。在「食材」页，排在「现有食材」下面。
  ...
  - 只算**按重量买的**（有 kg/g/斤）：「1 盒」「2 袋」折不成克数，跟着算只会得出假精度。调料油水（`NOT_INGREDIENT`）不算"吃的量"。
  - UI 刻意和「采购统计」做成一套：一样的周/月切换、一样的条形分期（点一期只看那一期）、一样的「大类饼图 + 明细展开」。两张卡是同一个问题的两面——一个是花了多少钱买，一个是真吃掉了多少。
  - 勾「吃完」和拖滑条之后当场重算（它们正是这张卡的输入），不用等下次进页面。
  - 一点状态都没有时**不能凭空造数**：显示「还没有可统计的量：在『现有食材』里勾掉吃完的，或者拖一下剩余量滑条」，已加断言 `eatenRows().length === 0`。
  - 实测（模拟勾/拖之后）：按周分出 4 期、按月分出 8 月 9.812kg / 9 月 3.437kg；大类 主食 53% → 蔬菜 32% → 肉类 7% → 蛋奶豆 5% → 水产 2%；点开蔬菜看到「粉红西红柿 1.864kg / 精品胡萝卜 748g / 生菜 600g…」；点某一期后只剩那一期的大类。
  - **已知口径限制**（写在这里免得以后当成 bug）：只覆盖"自己买回来做的"。在外就餐、别人请客、没记进采购的东西都不在内；勾「吃完」的日期是**点勾那天**，不是真正吃完那天。离线缓存与版本号升至 v157。

- 2026-09-04：**单独一个「肉」被认成了牛肉，应该是猪肉**（用户："那肯定不行啊，一般肉是指猪肉"）。v155 只修了展示名，没看分类本身——`foodIdForItem("肉")` 返回的是 `beef`。
  - 根因在兜底那一行：`foods.find(food => clean.includes(food.name) || food.name.includes(clean))`。**反向那一半**（目录名包含输入）对单字几乎必然误判——`"牛肉".includes("肉")` 成立，而牛肉在目录里排得早，就归给它了；「菜」会撞「生菜类」、「米」会撞「大米」，纯属看谁排在前面。
  - 两处一起修：① `leanPork` 补关键词「肉」——**整条规则排在牛/羊/鸡/鸭/鱼/虾之后**，所以「牛肉」「鸡肉」「鱼肉」仍被各自那条先截住，只有单独的「肉」和认不出种类的（肉末、肉夹馍、鲜肉月饼）落到猪肉；② 反向匹配只对 **2 个字以上**开放。常用单字（蛋/鱼/虾/面/米）别名表本来就覆盖了，不受影响。
  - 顺手补了两个会被「肉」误伤的：`蟹` → 水产、`肉桂` → 调料。
  - 连锁反应：「肉」从牛肉改判猪肉之后，它和「牛排」不再是同一种食材，**该分开列**——v155 那条"具体的赢笼统的"断言相应改成"牛排和肉各算一种"，另加一条"牛排和牛肉才该合并"来守住原本的意图。「本周吃到」的计数 30 → 28（更多写法归并到了正确的食材上）。离线缓存与版本号升至 v156。
...
- 2026-09-04：**换设备一键带同步配置**（用户："为什么这个项目会自动同步，但这个不会，我现在在电脑上打开"）。不是 bug：同步口令**按浏览器**存在 localStorage 里，不跟着网址走，所以新设备一律是"未配置"，只剩写死在 HTML 里那批数据（截图里停在 2026-07-30、「本周吃到 0 天」）。他另一个项目有服务端账号体系，登录即同步，这个是 GitHub Pages 上的单文件静态页、没有登录，两者的架构差别就在这儿。
  ...
  - **口令放在 URL 的 `#fragment` 而不是查询串**：fragment 不会被浏览器发给服务器，也不进 GitHub Pages 的访问日志。落地那边读完立刻 `history.replaceState` 从地址栏抹掉——不该留在历史记录里、也不该被顺手分享出去。
  - `applySyncLink()` **必须跑在 `initSync()` 之前**，否则这一次打开仍是"未配置"，要再刷新一遍才生效（已加断言锁住顺序）。剪贴板在非 https 或权限被拒时用不了，那就把链接摊在一个只读文本框里让人自己长按复制。
  - 口令绝不入库：链接是在用户设备上现生成的，已加断言禁止仓库里出现写死的口令。
  - 实测：已配设备生成链接 → 清空 localStorage 的全新设备用该链接打开 → 地址和口令都已存下、`syncConfig()` 为真、地址栏的 `#sync=` 已抹掉。离线缓存与版本号升至 v150。

- 2026-08-17：**大批食材落进「其他」：小票写的是部位名和品种名，不是目录里的类名**（用户："上脑不是肉吗…好多都不对，尽量不要放在其他里"、"茄子被识别了两次，名字还不一样——长茄和线茄"）。
  - 扩了 18 条规则。**肉**：上脑/眼肉/西冷/沙朗/菲力/板腱/匙仁/匙柄/保乐肩/肥牛（几乎专属牛肉，且必须排在 `leanPork` 前面，否则「牛小排」会被那边的「小排」抢走）、前腿/后腿/大排/肋排/龙骨/筒骨/汤骨/蹄髈/肉末/肉糜/绞肉/培根/火腿/腊肉、鸡胗/鸡架、黄鱼/带鱼/鲳鱼/多宝鱼。**菜**：`eggplant` 加单字「茄」兜住长茄/线茄（**必须排在 tomato 之后**，「番茄」里也有个「茄」）；`tomato` 加千禧/串收/樱桃番茄/小番茄；`leafy` 兼作「绿叶茎菜」通用桶（芹菜/莴笋/苦菊/菜心/香菜/豆芽/秋葵/芦笋…）、`radish` 兼作「根茎菜」通用桶（山药/莲藕/荸荠/茭白/竹笋），落这儿至少能进「蔬菜」组，比掉进「其他」强；紫甘蓝、木耳银耳、杭椒/线椒、佛手瓜/节瓜。**主食**：馒头/花卷/包子/饺子/馄饨/年糕/河粉/粉条归 `noodle`，糯米/黑米/粳米归 `rice`。
  - **去重那一道改成按 `foodId` 对，不按品名**：同一行在两张重叠截图上被读成「长茄」和「线茄」，按名字永远抓不住。认不出来的（`custom:`）仍退回按名字，否则一堆 custom 会互相误伤。实测：长茄/线茄同价同规格 → 合成一条；长茄 ¥4.9 + 圆茄 ¥6.2 → 一条不删。
  - 存量数据自愈：`healManualFoodIds()` 启动时重推导所有手动行的 `foodId`，跑在去重之前，所以老数据也会跟着归好类、该合并的合并。
  - 副作用记一笔：「火腿」原先落在 `custom:` 里自成一种，现在认成猪肉，于是「本周吃到」从 31 种降到 30 种——断言一并更新。这类"分类变准了导致计数变化"的连锁反应，正是那两条硬编码断言存在的意义。离线缓存与版本号升至 v149。
...
- 2026-08-17：**拿真 key 实调了一次，挑模型的过滤条件按真实返回收紧**。此前所有"实测"都是打桩的逻辑分支（key 只在用户设备上，仓库和 agent 都没有）；用户临时提供 key 后做了三件桩测做不到的验证：① `ListModels` 真实返回 **53 个模型、39 个支持 generateContent**，字段结构和假设一致；② `callGemini` 走完整挑选逻辑真实调用成功，挑中 `gemini-flash-lite-latest`（实际解析到 `gemini-3.5-flash-lite`），返回合法 JSON，第二次调用直接复用记住的那个、不再问 ListModels；③ 顺带确认**当时配额已经重置**（HTTP 200 而非 429），一张图约 1100 token。
  ...
  - key 用完即弃：测试脚本和拉回来的模型清单都已删除，**key 不写进任何文件、不入库、不发布**，已提醒用户去 AI Studio 轮换。离线缓存与版本号升至 v144。
...
- 2026-08-17：**两个 provider 各存各的 key，一个挂了自动顶上**（用户："现在都根本不识别了，很糟糕"）。先确认过不是代码回归：CDP 里把底层模型调用打成桩，采购小票和餐食照片两条链路都正常记录、队列清空、无异常。真正的原因就是截图里那个 **Gemini 429（免费额度用完）**，而当时的设计下用户是没有退路的——`nutriflow_ai_key` 只有一个格子，**换 provider 就把另一家的 key 冲掉**，所以"先用通义千问顶一下"的代价是回头还得重新去翻 Gemini 的 key。
  ...
  - 新增 `callModel(prompt, base64, mime)`：主力抛错就看另一家有没有 key，有就换它再试一次，**成功后把备胎记成当前 provider**——否则每张照片都要先在挂掉的那家上白撞一次。`recognizeReceipt` 和 `recognizePhoto` 都改走它，provider 分支从两处收敛到一处。
  - 存完 key 会把队列里所有任务的退避清零并立刻补跑；队列提示里也加了「立刻重试」按钮——退避是按分钟算的，人明明知道额度回来了却只能干等，比等本身更难受。
  - 实测（Gemini 桩永远 429、通义千问桩正常）：上传小票 → 自动顶到通义千问 → 正常记下商品 → `provider` 已切成 `qwen` → 队列清空。离线缓存与版本号升至 v140。
...
- 2026-08-17：**采购的每个字段都能改 + Gemini 模型名过期后能自愈**。离线缓存与版本号升至 v138。
  ...
  - **整单的店名和日期也能改了**。它们每行各存一份，所以 `updateManualReceipt()` 一次改这一单所有行；日期只换前 10 个字符，**时分要留着**——采购历史按它排序，也看得出是早上还是晚上买的。
  - **Gemini 报错**（用户："现在上传不了图了，说 gemini 报错"）。`GEMINI_MODELS` 是写死的两个名字，过期之后整个识别就哑了。改成：候选全试完还不行，就问一次 `ListModels`，把这个账号当下真能用的通用 flash 接在队尾**再试一次**，成功就记住。三个细节：① 必须**用下标遍历**，`for...of` 会在开头把数组定死，追加进去的候选根本轮不到；② 400 也当作"这个模型不行"继续换，不是只有 404；③ 超时（AbortError）和"连不上"分开说——前者是发出去了没回来，照片和任务都留着会自动重试。
...
- 2026-08-17：**每周计划的食材 chip 去掉「其他」整段**（用户："牛油果不显示在每周计划的编辑里，其他这个部分就不要了"）。`planIngredients()` 加一道 `filter(chip => chip.rank < 2)`，分段标签也从三个减到两个。写计划想的是"配什么菜、用哪块肉"，牛油果、米、坚果不在这个脑回路里。**「现有食材」那张卡仍然是三段**——那边是清库存，看得全才有用，已加断言分别锁住两处。实测：菜 15 样、肉 5 样、其他 0 样。离线缓存与版本号升至 v134。
...
- 2026-08-17：**底栏顺序改成 饮食 → 采购 → 食材 → 营养 → 计划**（用户要求）。采购和食材对调：先记账再看库存，跟实际动作的先后一致。断言里的顺序一并改掉。离线缓存与版本号升至 v131。

- 2026-08-17：**在外就餐缩略图收窄、照片真按餐次分、各类均价、增量补价**。离线缓存与版本号升至 v130。
  - **缩略图从 86px 网格改成 54px 一排**（用户："乱七八糟"）。九笔记录、每笔三四张大图，一屏只能看一条。在外就餐这里的图只当索引用：小方块横向排，多了横滑，点开照样是大图。只收窄 `.dining-entry` 里的——饮食页那边是"看今天吃了啥"，图大一点有意义。
  - **照片按餐次分，这次是真的分了**（用户："你现在好像还是没去分午餐和晚餐的图片"）。上一版留了个退路"这一天要是一张带餐次标记的照片都没有，就全借"，结果**老照片全都没标记，于是每天都走退路，等于没改**。现在规则收紧成：这笔知道是哪一顿，就只借标记对得上的；一张都对不上就一张不借。同时补两处打标：`savePrivatePhoto` 存的时候写，`autoRecognize` 认出来之后用 `stampPhotoMeal()` 再写一次（老照片重新识别也能补上标记）。代价是**没标记的老照片不再被借过来**——但借错比不借更糟。
  - **各类平均单价**（用户："对比下肉类、水产，等的平均单价，就是说这类别的均价"）。`categoryUnitPrices()` 排在单价对比的分类筛选**之上**且不跟着筛选变——它存在的意义就是横向比几类的贵贱。算法是「这一类的总金额 ÷ 总重量」，不是各食材单价的算术平均：后者会让 150g 的金针菇和 5kg 的米一样重。实测：水产 80.9 → 肉类 40.0 → 蛋奶豆 17.4 → 蔬菜 6.0 → 主食 5.8 元/kg。
  - **照片可以增量补到已有的那笔上**（用户："我会上传一些照片，你自己增量去补充里面的信息，有可能是一顿已经记录的饭的价格"）。识别完之后跑一遍 `mergeDiningWithExisting()`：同一天同一顿要是已经有一笔，就并进去而不是并排放两条。**只在两条互补时才合并**——同一个字段两边都填了且值不一样，那多半真是吃了两顿，不动。合并时价格/店名谁有算谁的，`dietEntries` 的 `diningId` 和 IndexedDB 里照片的 owner 都跟着改挂，临时那条删掉。
  - **「待补价」去掉**（用户："不用写待补价，我自己会看的"），卡片头上的「N 笔待补价」也一起去掉。

- 2026-08-17：**在外就餐整块重做、折叠头一律整行可点、食材卡片瘦身**。离线缓存与版本号升至 v129。
  - **在外就餐改成拍照优先**（用户："页面太丑了，不要上来就让填，一样上传照片"）。和采购、饮食一个规矩：一个大的「📷 拍张照就行」+ 缩略图，手填收进「或者手动填」的折叠里。选完照片直接建一笔、存图、后台识别（连拍 1.6s 内的算同一笔），店名和吃了啥从照片里读，识别进度写在原来那段说明小字的位置上。
  - **编辑图标换成线条铅笔**（用户："编辑的小icon太丑了，就用那种小笔，看下prettier的编辑icon"）。emoji ✏️ 在 iOS 上是彩色立体的，和这一页的线条感是两回事；改成和顶栏设置图标同一套的内联 SVG（`currentColor`）。
  - **价格不再独占一行**（用户要求）。一条记录压成一行：店名 +「¥价格 · 餐次 · 日期 · 照片」+ 铅笔；点铅笔才在下面展开一行可编辑的店名/日期/价格/删除。
  - **照片按餐次借，不再整天一起拉**（用户："把不是在外就餐的那张图也拉过来了，要区分下是午餐的图还是晚餐的图"）。根因是照片只按 `diet:日期` 归属，压根没有餐次这个信息。现在 `savePrivatePhoto(owner, file, meal)` 把餐次写进照片记录，饮食页存图时传人选的那一顿；`diningMealOf(entry)` 反查 `dietEntries` 里带 `diningId` 的那条得到餐次，只借餐次相同的。**老照片没有标记**——那一天要是一张带标记的都没有，就维持原样全部借过来，免得历史记录突然空掉。
  - **所有折叠头整行可点**（用户："每个展开都是点区域内任何位置都能展开"）。抽出 `bindFoldHead(buttonId, key, isFolded, apply)`，六处折叠（计划/每周目标/现有食材/单价对比/统计/采购历史）统一走它。**监听只挂标题行、不挂按钮**——按钮的点击会冒泡上来，两处都挂会连点两次互相抵消。现有食材分块的箭头收进 `.fold-caret`（9px、半透明），别抢标题。
  - **食材卡片去掉「建议频率」那行**、鸡肉那条去掉「三者差异不值得拆开评分」（均为用户要求）。`weeklyTimes` 数据保留，「每周采购目标」还在用。
  - **桃子和百香果搜不到**（用户："我吃了这么多次桃子和百香果，你都没加进去"）。其实分类早就覆盖了（桃 → `stoneFruit`、百香果 → `tropical`），但食材库显示的是**类名**，`variants` 里写的是「桃 / 油桃…」和「芒果 / 木瓜 / 菠萝」，搜「桃子」「百香果」一个都匹配不上。把 variants 改成用户实际会说的词：「桃子 / 油桃 / 李子 / 杏 / 樱桃」「芒果 / 百香果 / 木瓜 / 菠萝 / 火龙果」。
  - 每周计划标题右边的「点一天就能写」按用户要求删掉。
