...
- 2026-07-23 由用户上传的两张餐食照片识别：午餐为鸡胸肉煎排+两颗肉丸+胡萝卜+毛豆+花菜（花椰菜/西兰花混合，沿用旧日同款菜品的 `花菜` 写法）+藜麦紫米饭；晚餐为砂锅米线（煎蛋、炸豆皮、生菜、米线）。煎蛋按 `煎蛋` 写入（含 `蛋` 关键词，正确落到蛋奶豆）；`豆皮` 不匹配任何分类关键词，会归到未分类、排在餐次末尾，本周汇总里也不计入种类数。
