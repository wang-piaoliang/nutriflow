...
- 2026-07-27（用户文字录入，今天 07-28 的「昨天」）：午餐在外「北京炸酱面」（没给店名，未加 `place`）；加餐「满记甜品」记为 `{name:"加餐", place:"满记", items:["甜品"]}`；晚餐在家 `{name:"牛排", as:"牛肉"}`（「牛排」不含分类关键词，用 `as` 归鱼禽瘦肉）、鸡胸肉、胡萝卜、白萝卜、毛豆米、藜麦小米大米饭。这是首条 `加餐`，`MEAL_ORDER` 里加餐排最后。
