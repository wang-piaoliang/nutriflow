...
- Direct meal photos in this thread (2026-07-24 user convention): the user may drop meal photos straight into the Claude Code thread with no text. One photo = the single most recent meal; two photos = the two most recent meals, ordered earliest-first (first photo is the earlier meal). Infer the date and 早/午/晚餐 from the current date and the photos (default to the same day's most recent meal; roll back to the previous day when the timing implies it), write them into `dietRecords`, state clearly in the reply which day and meal each was logged as so the user can correct in one word, then update `NUTRIFLOW_PROJECT_CONTEXT.md` and publish. Publishing is the default and needs no extra confirmation. See the 饮食 food-naming rules below (e.g. eggs as `蛋`).
- Eating out (2026-07-24 user convention): when a meal was eaten at a restaurant, set the meal's `place` to the restaurant name — it renders first, inline with the food, as `名称：` (`.meal-place`, bold, a colon and no 📍 icon per the user), and survives record merging in `allDietRecords()`. List what was eaten as short dish names, not just raw ingredients (e.g. `宫保鸡丁`, `罗宋汤`), kept short. Use the `{name, as}` hidden-tag form when a dish name lacks a category keyword so it still classifies and dedupes in 本周吃到. Home-cooked meals keep listing plain foods with no `place`. (Example: the 2026-07-24 寿司郎 dinner uses `place:"寿司郎"`.)
...
- Preserve the four-tab order: `饮食`, `采购`, `食材`, `目标`. `饮食` is the default landing view (its `<section>` and first nav button carry `active`). The last tab is the former `首页`, now renamed `目标` (daily/per-meal targets and the food pyramid); keep `data-view="home"` / `id="home"` unchanged.
