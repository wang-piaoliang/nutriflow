...
- Eating out (2026-07-24 user convention): when a meal was eaten at a restaurant, set the meal's `place` to the restaurant name — it renders first as `📍 名称` above the food (`.meal-place`), and survives record merging in `allDietRecords()`. List what was eaten as short dish names, not just raw ingredients (e.g. `宫保鸡丁`, `罗宋汤`), kept short. Use the `{name, as}` hidden-tag form when a dish name lacks a category keyword so it still classifies and dedupes in 本周吃到. Home-cooked meals keep listing plain foods with no `place`. (Example: the 2026-07-24 寿司郎 dinner uses `place:"寿司郎"`.)
